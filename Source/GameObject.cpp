    ...
    GameObject GameObject::ButterflySubSpaceDivision()
  	{
  	    ...
  	    //EdgeFaces efs[el];
  	    //ControlPoints cps[fl];//removed as i don't need this out of the scope of the face loop
  #ifdef PrintToConsole
	    ...
	    ControlPoints cps[3];
  		...
  		for(int ei = 0; ei < 3; ++ei)
  		{
			...
			{
			if(fi % 8 == 0 || fi % 7 == 0  ||  ei != 2)//TODO: Remove this hacky amobination......
	  ...
	  control point positions labeled for reference
			...
			//a1 /a[0]
			cps[ei].a[0] = farr[fi].earr[ei].a;
			//a1 /a[0]
//			cps[fi].a[0] = farr[fi].earr[ei].a;

			//a1 /a[1]
			cps[ei].a[1] = farr[fi].earr[ei].b;
			//a1 /a[1]
//			cps[fi].a[1] = farr[fi].earr[ei].b;

			//b1 /b[0]
			cps[ei].b[0] = farr[fi].LocateFinalVertex(farr[fi].earr[ei]);
			//b1 /b[0]
//			cps[fi].b[0] = farr[fi].LocateFinalVertex(farr[fi].earr[ei]);

			//b1
			Face f1 = ef.FindFace(farr[fi].earr[ei], farr[fi]);
			cps[ei].b[1] = f1.LocateFinalVertex(farr[fi].earr[ei]);

			//c1 - neighbors f10 - b1 and a1 are known
			Edge e1 = farr[fi].LocateEdge(cps[ei].a[0],cps[ei].b[0]);
			Face f10 = ef.FindFace(e1,farr[fi]);
			cps[ei].c[0] = f10.LocateFinalVertex(e1);

			//c2 - neighbors f2 - b1 and a2 are known
			Edge e2 = farr[fi].LocateEdge(cps[ei].a[1],cps[ei].b[0]);
			Face f2 = ef.FindFace(e2,farr[fi]);
			cps[ei].c[1] = f2.LocateFinalVertex(e2);

			//c3 - neighbors f2 - b2 and a2 are known
			Edge e3 = f1.LocateEdge(cps[ei].a[1],cps[ei].b[1]);
			Face f5 = ef.FindFace(e3,f1);
			cps[ei].c[2] = f5.LocateFinalVertex(e3);
			//c4 - neighbors f7 - b2 and a1 are known
			Edge e4 = f1.LocateEdge(cps[ei].a[0], cps[ei].b[1]);
			Face f7 = ef.FindFace(e4, f1);
			cps[ei].c[3] = f7.LocateFinalVertex(e4);

#define HardDebug
      #ifdef HardDebug
			cout << "a[0] Located :"<< cps[ei].a[0].ToString() <<"\n";
			cout << "a[1] Located :"<< cps[ei].a[1].ToString() <<"\n";
			cout << "b[0] Located :"<< cps[ei].b[0].ToString() <<"\n";
			cout << "b[1] Located :"<< cps[ei].b[1].ToString() <<"\n";
			cout << "c[0] Located :"<< cps[ei].c[0].ToString() <<"\n";
			cout << "c[1] Located :"<< cps[ei].c[1].ToString() <<"\n";
			cout << "c[2] Located :"<< cps[ei].c[2].ToString() <<"\n";
			cout << "c[3] Located :"<< cps[ei].c[3].ToString() <<"\n";
      #endif

			//this list is built to help shared edges find new points.
			fs->nvs[ei] = ButterflyCalculateNewVertex(cps[ei]);
      ...
      #ifdef PrintToConsole
			cout << " New vertex calculated " << fs->nvs[ei].ToString() << "\n";
      #endif
			}
		    else
			{		    //end if forward
			...
			for(int i = 0; i < ec; ++i)
  			...
  			cout << "face finished \n\n";
  #endif
  			std::cout << "about to create new arrays \n";

  			Edge ne[9];
  			Face nf[4];

  			std::cout << "about to create new edges \n";

#ifdef HardDebug
for(int i = 0; i < 3; i++)
  	std::cout << "old vertex "<< i <<"/3  "<< fs->ovs[i].ToString()<< "\n";
for(int i = 0; i < 3; i++)
	std::cout << "new vertex "<< i <<"/3  "<< fs->nvs[i].ToString()<< "\n";
			...
			ne[7] = Edge(fs->ovs[1], fs->nvs[2]); //center right
			ne[8] = Edge(fs->nvs[0], fs->nvs[2]); //center left

			std::cout << "about to create new faces \n";
			...
			std::cout << "faces allocated to faces vector\n";
...
*/			std::cout << "about to allocate to faces vector\n";
  			/*
